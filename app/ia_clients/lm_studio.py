...
class LocalIAError(RuntimeError):
    def __init__(self, status_code: int, message: str):
        super().__init__(f"Local IA request failed ({status_code}): {message}")
...
class LocalIAClient:
    ...
    def generate_structured_output(self, prompt: str, response_schema: Type[T]) -> T:
        ...
        try:
            ...
            raise LocalIAError(exc.code, response_body) from exc
        except error.URLError as exc:
            raise LocalIAError(503, str(exc.reason)) from exc
        ...
        try:
            ...
            raise LocalIAError(502, f"Invalid structured response: {response_body}") from exc
